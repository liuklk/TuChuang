...
public class CouponUseAdapter extends BaseAdapter{
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView==null){
            convertView= LayoutInflater.from(mContext).inflate(R.layout.item_coupon_use_view,parent,false);
    ...
    class ViewHolder implements View.OnClickListener {
        ...
        public void setData(StoreBeen storeBeen,int position) {
            this.mStoreBeen=storeBeen;
            select.setVisibility(storeBeen.isCheck()?View.VISIBLE:View.GONE);
        }

        @Override
        public void onClick(View v) {
            if (v.getId()==R.id.expend){
                if (isExpend){
                    startAnimFold();
                }else {
                    startAnimExpend();
                ...
                if (mStoreBeen.isMyCoupon()) return;
                if (count%2==0){
